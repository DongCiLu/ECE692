...
class ConvolutionalNetwork(SupervisedModel):
        ...
        for i, l in enumerate(self.layers.split(',')):
            ...
            if node_type == 'conv2d':
                ...
                if first_full:  # first fully connected layer
                    ...
                    # if self.pre_W != None:
                        # print("setting weights based on input")
                        # W_fc = self.weight_variable_with_pre_W([fanin, dim])
                    # else:
                        # W_fc = self.weight_variable([fanin, dim])
                    W_fc = self.weight_variable([fanin, dim])
                    print('***The size of the fc layer is {}'.format(W_fc.get_shape()))
    ...
    def weight_variable_with_pre_W(self, shape):
